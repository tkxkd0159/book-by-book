...
- `users` stores:
  ...
  - internal auth data for internal admins:
    - `password_hash` (nullable, only used when `provider = 'internal'`)
- OAuth authentication and Book by Book signup are separate states:
  - a public user row may exist before signup is complete
  - `signup_completed_at` distinguishes authenticated-but-incomplete public accounts from usable public app users
...
- `favorite_genres` stores stable enum-like keys even though the UI groups Fiction and Non-Fiction separately and renders human labels.
...
- `max_uses = null` means unlimited use; `expires_at = null` means no expiry.
...
- Failed internal admin sign-in attempts are throttled with shared rate-limit infrastructure:
...
- Persist app user identity independently from provider email:
  ...
  - internal email identifies internal admins only and is globally reserved across `users`
...
- Favorite genres:
  ...
  - stored values:
    - Fiction: `FANTASY`, `SCI_FI`, `MYSTERY_CRIME`, `THRILLER_SUSPENSE`, `ROMANCE`, `HISTORICAL_FICTION`, `HORROR`, `LITERARY_FICTION`
    - Non-Fiction: `BIOGRAPHY_AUTOBIOGRAPHY`, `MEMOIR`, `HISTORY`, `TRUE_CRIME`, `PERSONAL_DEVELOPMENT`, `SCIENCE`, `PHILOSOPHY`, `TRAVEL`, `BUSINESS_ECONOMICS`, `COOKING_FOOD`, `ESSAYS_JOURNALISM`
  - UI labels continue to use the current English genre copy
...
- Invitation code creation:
  ...
  - raw code value is generated by the server with cryptographic randomness
  - display format is `XXXXX-XXXXX-XXXXX-XXXXX`
...
- `User.email` unique for non-null emails
...
- Raw invitation codes are system-generated as readable Crockford-style uppercase codes, shown once at creation, and then only the hash remains persisted.
